...
|---|---|---|
| **Torso swing** (`FormError.torsoSwing`) | Biceps curl — **NOT EMITTED by current side analyzer** (legacy only) | **Status (2026-05-21):** the side analyzer no longer emits this error. Lateral momentum in the user's frontal plane projects into screen depth in a 2D side-view camera and is geometrically unobservable. The lateral-X leg of the prior implementation was firing on what was *actually* forward sagittal lean (because in side view the lateral and depth axes are swapped under projection); that path was deleted and the lean-angle leg renamed to `FormError.depthSwing` (see below). The enum value is retained for legacy session deserialization (`FormError.values.byName('torsoSwing')` keeps working) and for potential future front-view re-introduction. TTS string ("No swinging") and UI labels are still wired but no live code path currently produces this error. |
| **Depth swing** (`FormError.depthSwing`) | Biceps curl — **side view (current behavior, 2026-05-21+)** | Forward sagittal trunk lean. Emitted by `CurlSideFormAnalyzer` when `\|currentTrunkAngle − baselineTrunkAngle\| > FormThresholds.torsoLeanThresholdDeg` (default `kTorsoLeanThresholdDeg = 12°`). The cue strings ("Don't rock forward" / "Rocking Forward") have always matched forward lean and remain unchanged. **Historical note:** before 2026-05-13 this enum value identified a *front-view* sagittal-sway detector built on a composite of scale-invariant torso features (`Sagittal Sway Detector`, `Arm-Over-Torso Occlusion Gate`, `Head Stability Corroborator`). That front-view detector and all three of its supporting modules were deleted with the rest of the front analyzer on 2026-05-13. The enum value was repurposed 2026-05-21 for the side-view forward-lean cue (formerly the lean-angle leg of `torsoSwing`) so the cue name finally matches the math. Highlights both shoulders. Quality deduction via the lean leg of `_computeQualityScore` (`_maxLeanDeltaDeg`, capped at `kQualitySwingMaxDeduction` = 0.25). |
| **Shoulder arc** (`FormError.shoulderArc`) | Biceps curl (side views only) | Hip-pivot rotation around the vertical axis. Computed as the **X-component only** of the shoulder displacement in hip-relative coordinates: `\|relX_current − relX_baseline\| / torsoLen > kSwingThreshold` (0.25), where `relX = shoulder.x − hip.x` cancels whole-body translation so only the rotational component remains. The Y component of the displacement is intentionally dropped — a forward sagittal lean ALSO moves the shoulder in Y (the torso pitches forward, lowering the shoulder in screen space), and including it would re-introduce the lean confound that already lives on the `depthSwing` detector. **Math change 2026-05-21:** the metric used to be the Euclidean magnitude `sqrt(dx² + dy²) / torsoLen`, which was direction-blind and false-fired "Stop rotating" at ~14° forward lean (the user's 2026-05-21 regression report). Cue: "Stop rotating". Highlights both shoulders. Reuses the lateral-swing quality budget (`kQualitySwingMaxDeduction`). **Audit bar:** the post-session Form Audit "Shoulder drift" criterion now grades against `swingThreshold` (0.25) — matching the live cue's bar; previously it borrowed `driftThreshold` (0.20) from a time when the metric was a direction-blind generic "shoulder moved from rep-start" scalar. Bar mismatch was fixed 2026-05-21 alongside the geometry fix so the audit card no longer flags rows the user never heard cued live. |
...
| **Elbow drift** (`FormError.elbowDrift`) | Biceps curl (side view only) | Torso-perpendicular projection of `(E − S)` onto `n̂ = (−u_y, u_x)` where `u = (S − H)/|S − H|`, normalised by torso length. The perpendicular projection is invariant to torso lean by construction — a forward stance no longer fakes elbow drift. **INSTANTANEOUS per-frame fire** (2026-05-21): `evaluate()` emits `elbowDrift` on every frame whose `|ratio|` clears BOTH the effective dead-band AND `kDriftThreshold` (0.15) — the cue plays for as long as the elbow is off the torso line and stops the moment it returns. (History: per-frame → per-rep sustained-frame verdict 2026-05-16 → back to per-frame 2026-05-21 per user request.) Magnitude drives the flag; sign (`lastSignedElbowDriftRatio`, positive = elbow on the +n̂ side) is preserved in side-view telemetry (schema v5+) so analysis can split "elbow forward" (front-delt cheat) vs. "elbow back" (tricep-press) cheats. **Backward-drift coverage:** the elbow landmark for this detector uses a relaxed visibility floor `kCurlDriftElbowMinConfidence` (0.25) instead of `kMinLandmarkConfidence` (0.40) — ML Kit's elbow confidence drops more on backward drift because the curling bicep occludes the elbow; the relaxed gate restores forward/backward detection parity. Cue: "Keep your elbow still". |
...
| **No knee flexion** (`FormError.noKneeFlexion`) | Squat | Two firing paths after the 2026-05-21 broaden: (a) **Stiff-Legged Miss** (NEW, lone trigger): knee delta `(_startKneeAngle − _minKneeAngle) < kSquatNoKneeFlexionStiffLeggedMaxDeltaDeg` (15°) regardless of lean. (b) **Hip-Pivot Conjunction** (legacy): peak lean `_maxLeanDeg ≥ kSquatNoKneeFlexionMinLeanDeg` (25°) AND knee delta `< kSquatNoKneeFlexionMaxKneeDeltaDeg` (20°). The OR closes a hole where a 15°-lean / 10°-knee-bend rep tripped nothing. TTS cue: "Bend your knees". **Thresholds PRELIMINARY** — awaiting telemetry-derived tuning. |
| **Hips forward on descent** (`FormError.hipsForwardOnDescent`) | Squat | Hip drifted toward the toes (instead of hinging back) within the first `kSquatHipsForwardWindowMs` (200 ms) of DESCENDING. Computed as `(hip.x[t=window] − hip.x[t=0]) × forwardSign / leg_len_px > kSquatHipsForwardMinRatio` (0.05); `forwardSign` normalized from initial `sign(hip.x − heel.x)`. **TOMBSTONED 2026-05-21** — detector still computes for telemetry; FormError no longer emitted from the analyzer (small-window detector with fragile sign normalization; subsumed by `noKneeFlexion`'s broadened firing condition). Enum value retained for legacy session deserialization. |
| **Knee-led descent** (`FormError.kneeLedDescent`) | Squat | Knee-dominant initiation: `(|Δknee.x| / leg_len) / (Δhip.y_down / leg_len) > kSquatKneeLedMinRatio` (1.2) over the early-descent window. **TOMBSTONED 2026-05-21** — `kSquatKneeLedMinHipDropNorm` floor was never wired into the analyzer, denominator instability caused false fires on the user; subsumed by `noKneeFlexion`. Detector still computes for telemetry; FormError no longer emitted. Enum value retained. |
| **Knee-dominant pattern** (`FormError.kneeDominantPattern`) | Squat | Compound — peak `forwardKneeShift` AND peak `heelLift` in the same rep. **TOMBSTONED 2026-05-21** — both contributing detectors fire independently with the new terse cue vocabulary, so the compound message is redundant. Enum value retained. |

### 4.1h Squat audit cue surface (2026-05-21 trim)

The shipping squat cue vocabulary is **terse**: each TTS line names one body part and one direction. The view-model walks the analyzer's returned `List<FormError>` and `break`s at the first non-suppressed cue, so **list order = priority**. Curl shares the same `_onFormErrors` pipeline in `workout_view_model.dart:3254-3334`; squat changes are confined to the analyzer's emission order and the suppression set.

| # | FormError | TTS line | Notes |
|---|---|---|---|
| 1 | `squatDepth` | "Deeper" | Dominant cue. Half-squat veto also suppresses rep count on this. |
| 2 | `noKneeFlexion` | "Bend your knees" | Broadened — stiff-legged miss OR hip-pivot conjunction. |
| 3 | `hipLead` | "Chest up" | Y-velocity only — sign-immune by construction. Catches good-morning ~300ms earlier than the lean detector catches the symptom. |
| 4 | `heelLift` | "Heels down" | Frame-level emission (merged before completion errors), lands at priority 4 naturally. |
| 5 | `excessiveForwardLean` | "Chest up" | Sustained-fraction gate (0.35 of ≥6 frames). Sign-normalized 2026-05-21 against `sign(hip.x − heel.x)`. |
| 6 | `squatEccentricTooFast` | "Slow down" | Pure clock measurement. Concentric counterpart is tombstoned (explosive ascent is good form when unloaded). |

**Suppressed from TTS** (`workout_view_model.isTtsSuppressed`): `forwardKneeShift` (informational, visual highlight only); `excessiveBackwardLean` (telemetry only — instantaneous detection is jitter-prone and bodyweight backward lean is rare).

**Tombstoned detectors** (still compute for telemetry, no FormError emission): `hipsForwardOnDescent`, `kneeLedDescent`, `kneeDominantPattern`, `squatConcentricTooFast`, `squatTempoInconsistent`, `squatFatigue`. Commit-time snapshot getters preserved on the analyzer so the `squat.rep` telemetry line stays intact.

### 4.1i Sign-normalization contract (side-view 2D pose)

Any detector that consumes a raw x-axis delta in 2D side view **must** sign-normalize against the user's facing direction. The canonical anchor is the heel→hip X vector: `forwardSign = sign(hip.x − heel.x)`. For pure magnitudes where direction is irrelevant (e.g. knee shift), use `.abs()`. Audits added without this normalization are correct by accident for whichever facing direction the developer happened to think of; the failure mode is **silent inversion**, not a crash. Mirror-pose unit tests (mirror all landmarks across `x = 0.5`, swap confidence channels, assert same magnitude) are mandatory for new x-delta detectors. See `WISDOM.md` 2026-05-21 row.
...
> **⚠️ Deleted 2026-05-13** along with the rest of the front-view curl analyzer (`lib/engine/curl/sagittal_sway_detector.dart`, `head_stability_corroborator.dart`, `curl_form_analyzer.dart`). The detail below is retained as historical record because the design (composite scale-invariant features, 1€-filtered, z-scored against a baseline window) remains a useful reference if a front-view curl variant is ever revived. The `FormError.depthSwing` enum value was repurposed 2026-05-21 for a **side-view forward-lean** cue (trunk-angle-from-vertical delta) — see §4.1 rows for the current behavior. Sections §4.1c (Arm-Over-Torso Occlusion Gate) and §4.1d (Head Stability Corroborator) below are likewise historical.
...
**Fires** `FormError.hipLead` when `ratio > kHipLeadVelocityRatio` (1.4). Quality deduction: severity `= ((ratio − 1.4) / 0.6).clamp(0, 1) × kQualitySquatHipLeadMaxDeduction (0.15)`, applied multiplicatively in `_computeQualityScore` AFTER lean and heel-lift. TTS cue: "Lead with your chest" with the standard 1.5 s cooldown.
...
| Term | Meaning |
|---|---|
| **Feedback cooldown** | `kFeedbackCooldownSec` (1.5 s as of 2026-05-21, lowered from 3 s) minimum between two voice cues of the same type. Prevents spam while keeping per-frame cues (e.g. `elbowDrift`) responsive. Comparison sites use `.inMilliseconds` — `.inSeconds` would truncate the fractional 1.5. |
