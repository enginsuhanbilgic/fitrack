...
| **SummaryHero** | `widgets/summary/summary_hero.dart`. The "AI Form Accuracy" cyan-bordered top card shown on every Session Complete page. Uses the `FtAccentCard` paint pattern (uniform border + Positioned strip) to avoid the borderRadius-with-mixed-Border crash. Shows large quality % + grade pill + subtitle. |
...
| **FeedbackSensitivity** | `enum FeedbackSensitivity { high, medium }` in `core/types.dart`. **Unified sensitivity setting (2026-05-13)** — one toggle now controls coaching strictness across all exercises (curl, squat, push-up). Replaces the prior per-exercise `CurlSensitivity` / `SquatSensitivity` enums. High = tighter ROM gates + stricter form thresholds (75 % of medium for curl, −3° lean for squat); Medium = current defaults. Has no effect on calibrated or auto-calibrated paths — those are personal and already correct. Persisted via `preferences` table key `feedback_sensitivity`. |
| **FormThresholds** | Pure value class in `core/form_thresholds.dart`. Bundles all 6 form-error threshold constants (`swingThreshold`, `torsoLeanThresholdDeg`, `backLeanThresholdDeg`, `shrugThreshold`, `driftThreshold`, `elbowRiseThreshold`) into a single injectable object. `FormThresholds.medium` is bit-for-bit identical to the `k*` constants so existing behavior is unchanged. Created once in `WorkoutViewModel.init()` via `FormThresholds.forSensitivity(_feedbackSensitivity)` and injected down through `RepCounter → CurlStrategy → CurlFormAnalyzer / CurlSideFormAnalyzer`. |
...
| **FtAccentCard** | `core/theme.dart`. Canonical pattern for cyan-rule "AI surface" cards: uniform `Border.all(stroke)` + rounded corners + a `Positioned` left strip inside a `Stack`. Required because Flutter's `Border.paint` asserts that non-uniform-color borders cannot have a `borderRadius`. Any new accent card MUST use this pattern (or `SummaryHero`'s open-coded variant) — *not* `Border(left: BorderSide(color: cyan), …)`, which throws at paint time and renders an empty rectangle. |
| **_applyRomSensitivity** | Private static helper on `RomThresholds`. Applies additive ROM deltas to any base threshold set (from any tier) and re-derives `peakExitAngle = peak + kCurlPeakExitGap`. Includes FSM invariant floors so no sensitivity × view combination can produce an uncompletable FSM. Never called for the telemetry-defaults tier when a sensitivity-specific constant already exists. |
...
- **`_RowSparkline`** — `home_screen.dart`'s per-session sparkline widget. Renders `SessionSummary.qualitySeries` on a fixed 0..1 scale via `FtSparkline(minOverride: 0, maxOverride: 1)` — so a flat-good session draws flat instead of getting auto-amplified. For 0/1-point series (legacy rows, no-quality sessions) it falls back to a faint flat baseline so the row layout stays stable. Replaced a hard-coded placeholder array on 2026-05-13.
...
| Term | Meaning |
|---|---|
| **Form Audit** (concept) | The post-session retrospective grade. Re-applies the form-error and ROM thresholds to each rep's already-persisted maxes — but via **Path B-permissive tier priority (2026-05-13)** rather than a fixed strict bar: each rep grades against the most personalized ROM bar available (calibrated profile → auto-cal snapshot → cold-start at session sensitivity). Form-error thresholds use the user's session sensitivity. Always on, no toggle. Matches the industry standard (Strava / Whoop / Future) of grading against the user's own baseline. |
