...
| **FeedbackSensitivity** | `enum FeedbackSensitivity { high, medium }` in `core/types.dart`. **Uniform sensitivity contract (2026-05-14)** — one toggle controls coaching strictness across all exercises, applied as a post-pass at **every tier** for ROM thresholds (calibrated, auto-calibrated, cold-start) and for form-error thresholds. Pre-2026-05-14 it applied only at the cold-start tier for ROM gates; the broader scope was added to make the toggle remain meaningful after calibration. High = identity on the High anchor; Medium = High + per-tier looseness deltas (reproduces today's Medium-baseline numbers bit-for-bit at Tier 1 telemetry and Tier 3 cold-start; at bucket-derived tiers Medium now also loosens). Persisted via `preferences` table key `feedback_sensitivity`. |
